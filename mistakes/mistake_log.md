...
| 2026-09-03 | 2026-09-03-PM-R1 | Python | `dict.get()` and Key Existence | Does `record.get("crop_name") is None` prove that the key is absent? | — | — | Repeatedly treated a `None` return value as evidence that the key does not exist. | A present key may also have value `None`; use `"crop_name" in record` to test key existence. | 3 | Improved |
...
| 2026-09-05 | 2026-09-05-R1 | SQL | Window Frame / Peer Rows | What result can `SUM(amount) OVER (ORDER BY order_date)` produce when dates tie? | 100, 300, 350 | 300, 300, 350 | Applied a strict row-by-row running-total model despite tied `ORDER BY` values forming peer rows. | PostgreSQL's default peer-aware frame can give peers the same cumulative result; use deterministic ordering plus an explicit `ROWS` frame for strict row-by-row accumulation. | 2 | Improved |
